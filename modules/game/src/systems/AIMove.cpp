...
void AIMove::update()
{
    ...
    for (auto& [entity, nodes] : travelingEntities_)
    {
        ...
        const auto aiCollider
            = std::get<components::CircleCollider>(view.get<components::Collider>(entity));
        ...
        lastAiVelocity = {.x = aiVelocity.x != 0 ? std::abs(aiVelocity.x) / aiVelocity.x : 0,
...
void AIMove::onDetect(events::AIDetectedEnemy event)
{
    ...
    const auto aiCollider
        = std::get<components::CircleCollider>(view.get<components::Collider>(event.aiEntity))
              .position;

    const auto& pathfindingGraph
        = entityManager_.ctx().get<roen::data_structure::Graph<roen::data_structure::MapNode>>();

    if (auto it = travelingEntities_.find(event.aiEntity); it != travelingEntities_.end())
    {
        auto& destinationPosition = it->second.back().getPosition();

        if (Vector2DistanceSqr(destinationPosition, event.detectedEntityPosition) <= 1000)
        {
            return;
        }
    }

    const auto closestPlayerNode
        = getClosestMapNodeMaybe(event.detectedEntityPosition, pathfindingGraph);
    ...
    auto path = roen::algorithms::a_star(*closestAINode, *closestPlayerNode, pathfindingGraph,
                                         roen::algorithms::manhattanDistance);
    auto travelOrder
        = roen::algorithms::getNodeOrderFromPath(*closestAINode, *closestPlayerNode, path);

    travelingEntities_[event.aiEntity] = std::list(travelOrder.begin(), travelOrder.end());
}

std::optional<roen::data_structure::MapNode> AIMove::getClosestMapNodeMaybe(
    ...
    const roen::data_structure::Graph<roen::data_structure::MapNode>& pathfindingGraph) const
{
    ...
    while (!nodes.empty())
    {
        ...
        if (Vector2Distance(position, currentClosestNodeV)
            > Vector2Distance(position, consideredNodeV))
        {
